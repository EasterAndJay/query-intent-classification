...
def load_data_and_labels():
    ...
    train_path = "./data/TREC.train"
    if not os.path.exists(train_path):
        os.system("git clone https://github.com/easterandjay/query-intent-classification.git")
        os.system('mv query-intent-classification/data .')
        os.system('rm -rf query-intent-classification')
    x_text = list(open(train_path).readlines())
    # Split by words
    x_text = [clean_str(sent) for sent in x_text]
    y = np.asarray([s.split(" ")[0] for s in x_text])
